...
def capitalize_input(key):
    """Callback that capitalizes first letter of each word."""
    try:
        txt = st.session_state.get(key, "")
        if txt:
            st.session_state[key] = txt.title()
    except:
        pass
...
def smart_input(label, key_name, saved_dict):
    widget_key = f"{key_name}_box"

    # text input WITH auto-capitalization callback
    typed = st.text_input(
        label,
        key=widget_key,
        on_change=capitalize_input,
        args=(widget_key,)
    )

    # after user finishes typing, value is already capitalized by callback
    typed_cap = st.session_state.get(widget_key, "").strip()

    # Load saved values for suggestions
    options = saved_dict.get(key_name, [])
    matches = [o for o in options if typed_cap.lower() in o.lower()] if typed_cap else []
    ...
    # Auto-save new capitalized value
    if typed_cap and typed_cap not in options:
        options.append(typed_cap)
        ...
        if weight_mode == "Finish Goods Weight":
            ...
            if len(combo_items) > 1:
                ...
                for i, comp in enumerate(combo_items):
                    ...
                    per_piece_rate_combo = per_pc_wt * rate_per_kg
                    comp["per_piece_rate"] = per_piece_rate_combo

                    s1, s2, s3 = st.columns(3)
                    s1.success(f"{header_label} wt/pc: {per_pc_wt:.4f} kg")
                    s2.success(f"Per pc rate: ₹{per_piece_rate_combo:.2f}")
                    s3.success(f"{header_label} total ₹: {comp_price:.2f}")
                combined_per_piece_rate = 0.0
                 
                for comp in combo_items:
                    combined_per_piece_rate += comp.get("per_piece_rate", 0.0)
                st.markdown("---")
                st.success(f"⚖️ Combined per-piece weight: {total_weight_pc:.4f} kg")
                st.success(f"💰 Combined Per Pc Rate: ₹{combined_per_piece_rate:.2f}")


                # Use update_rate_and_next to write the total price into Rate for the selected_desc
                if st.button("➕ Add Value", key="add_value_combo"):
                    per_piece_str = f"{combined_per_piece_rate:.2f}"
                    update_rate_and_next(selected_desc, per_piece_str)
                    st.success("Per-piece rate added to table successfully!")
            ...
            # -------------------------
                ...
                if calculate_pressed:
                    if 'rate_value' in locals() and rate_value:
                        ...
                        sum_c3.success(f"Per Pc ₹: {per_piece_price:.2f}")
                        sum_c4.success(f"Total ₹: {total_price:.2f}")
                ...
                if add_value_pressed:
                    ...
                    try:
                        if 'rate_value' in locals() and rate_value:
                            ...
                            total_to_write = f"{per_piece_price:.2f}"
